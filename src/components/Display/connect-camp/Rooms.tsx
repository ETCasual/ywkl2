...
type RoomsProps = {
  rooms: Record<string, Record<number, string[]>>;
  bg: string;
};

export const Rooms: FunctionComponent<RoomsProps> = ({ rooms, bg }) => {
        ...
        <div className="flex w-full flex-row-reverse items-center justify-between border-b-2 border-black bg-[#96ec00]">
          <div className="flex flex-row-reverse items-center justify-center">
            <img
              ...
              alt="Warning_Rules"
            />
            <h2 className="mr-3 text-xl font-bold">营会房间 Rooms</h2>
          </div>
          <div
      ...
      <div className="flex h-[calc(100dvh-118px)] w-full flex-col items-center gap-7 overflow-y-scroll p-7">
        {Object.entries(rooms)
          .filter(([, roomOccupants]) => {
            const matchingMembers = Object.values(roomOccupants)
              .flat()
              .filter((ro) =>
                ro.toLowerCase().includes(searchString.toLowerCase()),
              );

            return matchingMembers.flat().length > 0;
          })
          .map(([level, roommates], i) => (
            <div
              key={i}
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center gap-3 p-5">
                ...
                {Object.entries(roommates)
                  .filter(([, rm]) => {
                    // console.log(gm);
                    const matchingMembers = rm.filter((roommate) =>
                      roommate
                        .toLowerCase()
                  ...
                  .map(([no, gm], idx) => (
                    ...
                    >
                      <div className="w-full border-2 border-black bg-[#ff6511] font-made text-sm">
                        <p className="px-5 py-2 text-lg capitalize">{no}</p>
                      </div>
